...
`make doctor`는 `claude`, `uvx`, `.env` 필수값을 확인합니다. Claude Code가 Azure OpenAI Responses API 경로를 쓰는 경우가 있어서 `AZURE_API_VERSION`은 `2025-03-01-preview` 이상이어야 합니다.

## Claude Code 실행 중 400 오류가 나는 경우

아래 오류가 보이면 `.env`의 `AZURE_API_VERSION`이 낮은 상태입니다.

```text
Azure OpenAI Responses API is enabled only for api-version 2025-03-01-preview and later
```

`.env`를 수정한 뒤 프록시 터미널에서 `Ctrl-C`로 종료하고 다시 실행하세요.
